...
import { state, me, farm } from './store.js';
import { esc, stageName, topbar, progressBar, hydratePhotos, photoImg } from './ui.js';
...
export function clientView() {
  ...
  const card = f => {
    ...
    return `<a class="card flat stack" href="#/client/farm/${f.id}" style="text-decoration:none;color:inherit">
      <div class="row between"><strong>${esc(f.id)} · ${esc(f.region)}</strong><span>${p}%</span></div>${progressBar(p)}
      ${approved ? `<span class="small muted">${approved} ${esc(t('photos_n'))}</span>` : ''}</a>`;
  };
  return {
    html: `<div class="screen wide">${topbar()}<main class="content">
      ${me().role === 'manager' ? mgrNav('client') : ''}
...
export function clientFarmView({ farmId }) {
  ...
  return {
    html: `<div class="screen wide">${topbar({ back: '#/client' })}<main class="content">
      ...
      <h1 class="h1">${esc(f.id)} · ${p}% ${esc(t('in_progress'))}</h1>
      <div>${esc(started)}</div>
      ${progressBar(p)}
