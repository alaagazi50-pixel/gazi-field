...
const TEXT: Record<string, { title: string; daily: string; followup: string; test: string }> = {
  en: { title: 'GAZI FIELD', daily: "Don't forget today's daily report.", followup: 'Management is waiting for today\'s report. Please send it.', test: 'Notifications are working.' },
  pt: { title: 'GAZI FIELD', daily: 'Não se esqueça do relatório diário de hoje.', followup: 'A gestão aguarda o relatório de hoje. Por favor envie-o.', test: 'As notificações estão a funcionar.' },
  ar: { title: 'GAZI FIELD', daily: 'لا تنسَ تقرير اليوم.', followup: 'الإدارة بانتظار تقرير اليوم. يرجى إرساله.', test: 'الإشعارات تعمل.' },
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let targets: string[] = [];
    let kind = '';
    let message: 'daily' | 'followup' | 'test' = 'daily';
    if (body.action === 'cron') {
    ...
    for (const s of subs || []) {
      ...
      try {
        await webpush.sendNotification({ endpoint: s.endpoint, keys: { p256dh: s.p256dh, auth: s.auth } },
          JSON.stringify({ title: t.title, body: t[message], url: './', tag: `gazi-${message}` }), { TTL: 6 * 3600 });
